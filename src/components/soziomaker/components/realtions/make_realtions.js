...
export default function MakeRelations(props) {
    ...
    const getChildIdByIndex = (idx) => {
        return currentDocument.rw_data.Childs[idx].id
    }

    const getRelationByChildId = (id) => {
        for (let index = 0; index < currentDocument.rw_data.Relations.length; index++) {
            const element = currentDocument.rw_data.Relations[index];
    ...
    const updateRelation = (rel, existing) => {
        if (!existing) {
            currentDocument.rw_data.Relations.push(rel);  
        }
        else {
            currentDocument.rw_data.Relations.forEach(r => {
    ...
    const onVotingElementClicked = (col, row, selected) => {
        ...
        let relation
        let exists = false;
        currentDocument.rw_data.Relations.forEach(rel => {
